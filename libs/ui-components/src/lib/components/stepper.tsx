...
export const Stepper = React.forwardRef<HTMLDivElement, StepperProps>(
  (
    ...
    const lineClasses = cn(
      'absolute left-0 top-2/4 h-0.5 w-full -translate-y-2/4 bg-slate-900',
      lineClassName
    );
    ...
    useEffect(() => {
      onLastStep &&
        typeof onLastStep === 'function' &&
        onLastStep(isLastStepValue);
      onFirstStep &&
        typeof onFirstStep === 'function' &&
        onFirstStep(isFirstStepValue);
...
export const Step = React.forwardRef<HTMLDivElement, StepProps>(
  (
    ...
    const stepClasses = cn(
      'relative z-10 grid place-items-center w-10 h-10 rounded-full bg-slate-900 text-slate-300 font-bold transition-all duration-300',
      className
    );
    return (
